...
for(( i=0;i<${#array[@]};i++)) do
    echo ${array[i]};
    sshpass ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no gtc@${array[i]} "bash ~/FastRoute/server/stop_server.sh"
done;
...
for(( i=0;i<${#array[@]};i++)) do
    echo ${array[i]};
    sshpass ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no gtc@${array[i]} "bash ~/FastRoute/client/stop_client.sh"
done;
